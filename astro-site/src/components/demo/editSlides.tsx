        ...
        <div className="flex flex-1 px-4 pt-2.5 pb-1.5 gap-3">
          <div className="flex-1">
            <svg viewBox="0 0 120 80" className="w-full h-full">
              <rect x="20" y="15" width="80" height="50" fill="#F3E5F5" />
              {[
                { x: 30, h: 35, label: 'Eng' },
                { x: 50, h: 25, label: 'Sales' },
                { x: 70, h: 40, label: 'Ops' },
                { x: 90, h: 20, label: 'Mkt' },
              ].map((bar, i) => (
                <g key={i}>
                  <rect x={bar.x} y={65 - bar.h} width="12" height={bar.h} fill={i % 2 === 0 ? PURPLE : VIOLET} />
                  <text x={bar.x + 6} y="75" textAnchor="middle" fontSize="4" fill="#595959">{bar.label}</text>
                </g>
              ))}
            </svg>
          </div>
        ...
        <div className="flex flex-1 px-4 pt-2.5 pb-1.5">
          <div className="grid grid-cols-3 gap-2 flex-1 content-start">
            {['Partner A', 'Partner B', 'Partner C', 'Partner D', 'Partner E', 'Partner F'].map((partner, i) => (
              <div key={i} className="aspect-square flex items-center justify-center" style={{ background: LAVENDER, border: `1px solid ${ORCHID}` }}>
                <span className="text-[5px] font-semibold" style={{ color: PURPLE }}>{partner}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="flex flex-col h-full" style={{ background: '#FFFFFF', fontFamily: "Calibri, 'Inter', sans-serif" }}>
        ...
        <div className="flex flex-1 px-4 pt-2.5 pb-1.5 flex-col gap-2">
          {[
            ...
            { title: 'Security Features', items: ['End-to-end encryption', 'MFA enabled', 'Regular audits'] },
          ].map((section, i) => (
            <div key={i}>
              <div className="text-[6px] font-bold mb-1" style={{ color: PURPLE }}>{section.title}</div>
              <div className="flex flex-wrap gap-1">
                {section.items.map((item, j) => (
                  <div key={j} className="px-2 py-1" style={{ background: LAVENDER, fontSize: '4.5px', color: VIOLET }}>
                    {item}
                  </div>
                ))}
              </div>
          ...
          <div className="grid grid-cols-2 gap-3 flex-1">
            {[
              ...
              <div key={i} className="text-center py-2">
                <div className="text-[14px] font-bold" style={{ color: PURPLE }}>{metric.value}</div>
                <div className="text-[5px]" style={{ color: '#757575' }}>{metric.label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="flex flex-col h-full" style={{ background: '#FFFFFF', fontFamily: "Calibri, 'Inter', sans-serif" }}>
        ...
        <div className="flex flex-1 px-4 pt-2.5 pb-1.5 flex-col gap-2">
          {[
            ...
            { dept: 'Product', count: '12 people' },
            { dept: 'Sales & Marketing', count: '28 people' },
            { dept: 'Operations', count: '15 people' },
          ].map((team, i) => (
            <div key={i} className="flex items-center gap-2">
              <div className="flex-1 px-2 py-1.5" style={{ background: LAVENDER }}>
                <div className="text-[5.5px] font-bold" style={{ color: PURPLE }}>{team.dept}</div>
                <div className="text-[4.5px]" style={{ color: '#757575' }}>{team.count}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-1 px-6 pt-3 pb-2 items-center justify-center">
          <div className="text-center">
            <div className="text-[8px] font-bold mb-2" style={{ color: PURPLE }}>
              Strong market position with 42% growth
            </div>
            <div className="text-[5px] leading-[1.5]" style={{ color: '#404040' }}>
              Ready for next phase of expansion in Q2 2026
            </div>
          </div>
        </div>
      </div>
